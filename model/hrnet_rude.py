...
class HighResolutionNet(nn.Module):
    def __init__(self, base_channel: int = 48, num_class: int = 2):
        ...
        self.relu = nn.ReLU(inplace=True)
        if base_channel==18:
            self.last_channel = 270
        else:
            self.last_channel = 720
        #v-18 最后多尺度拼接为270 v-48为720
        ...
        self.fuse = Bottleneck(self.last_channel, self.last_channel, downsample=None, expansion=1)
