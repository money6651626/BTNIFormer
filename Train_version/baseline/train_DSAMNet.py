...
from utils.train_model import train_one_epoch_DSAMNet
from utils.val_model import val_one_epoch_DSAMNet
import os
import random
import numpy as np
import torch
...
def get_args_parser(add_help=True):
    ...
    parser.add_argument('--net_G', default="DSAMNet_34", help='initial learning rate for adam')
    ...
    parser.add_argument("--init_weight", default=r"", type=str,
                        help="the init model path of first epoch")
    parser.add_argument("--output_dir", default=r"F:\Pycharm_program\lunwen\runs/weights_pretrain")
    ...
    parser.add_argument('--loss', default='bcl+dice', type=str)
    parser.add_argument('--optimizer', default='SGD', type=str)
    parser.add_argument("--tensorboard_log",default=r"F:\Pycharm_program\lunwen\runs/model_runs_pretrain/",type=str)
...
def main(args):
    ...
    if args.resume: #肯定不会是0 恢复的是latest.pth
        ...
        if args.init_weight:
            model.load_state_dict(torch.load(args.init_weight, map_location="cpu")["state_dict"], strict=False)
            print("Load pretrain_weight!")
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        epoch_eva_dict, epoch_loss = train_one_epoch_DSAMNet(model=model,criterion1=criterion1,criterion2=criterion2,device=device,dataloader_train=train_data_loader,optimizer=optimizer,
                                                            scaler=scaler,args=args)
        log_eva(epoch + 1, epoch_eva_dict, epoch_loss, writer_train)
        if lr_scheduler is not None:
            lr_scheduler.step()

        epoch_eva_dict, epoch_loss_val = val_one_epoch_DSAMNet(model=model, criterion1=criterion1,criterion2=criterion2, device=device,dataloader_val=val_data_loader,
                                                                args=args)
        ...
        if os.path.join(args.output_dir, args.log_message):
            ...
            # if epoch%5==0:
            #     torch.save(latest_checkpoint, os.path.join(args.output_dir, args.log_message, "{0}_ckpt.pth".format(epoch)))
        else:
